...
describe('Datepicker demo testing suite: Custom date format', () => {
    ...
    const currentMonthNum = newDate.getMonth();
    const currentMonthStr = datepicker.monthNames[newDate.getMonth()];
    const currentYearNum = newDate.getFullYear();
    ...
    it(`example contains 3 Datepicker inputs, 3 clickable buttons, value in inputs is current day in diff formats:
        ...
        datepicker.isInputValueEqual(customFormat, newDate.toLocaleDateString('Lt'), 0);
        datepicker.isInputValueEqual(customFormat, `${monthToChose}/${dayToChose}/${currentYearNum}`, 1);
        datepicker.isInputValueContain(customFormat, newDate.toLocaleDateString('en-US', { month: 'long', day: 'numeric' }), 2);
        datepicker.isInputValueContain(customFormat, `${newDate.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }).toLowerCase().split('')[1]}`, 2);
    });
    it('when user click on the first "Date Picker" btn, chose other date - it chosen, appear in "YYYY-MM-DD"', () => {
        ...
        datepicker.isInputValueEqual(customFormat, new Date(`${currentYearNum}-${currentMonthNum + 1}-${Number(dayToChose)}`).toLocaleDateString('Lt'), 0);
    });
    ...
    it('when user click on the third "Date Picker" btn, chose other date - it chosen, MMMM Do YYYY,h:mm:ss a', () => {
        ...
        datepicker.clickOnDatepickerTableItem('date', 'body', undefined, dayToChose);
        datepicker.isInputValueContain(customFormat, `${currentMonthStr} ${dayToChose}th ${currentYearNum}`, 2);
        datepicker.isInputValueContain(customFormat, `${newDate.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }).toLowerCase().split('')[1]}`, 2);
    });
});
//# sourceMappingURL=custom_date_format_spec.js.map