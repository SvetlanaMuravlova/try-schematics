...
describe('Datepicker demo testing suite: Initial state', () => {
    ...
    it(`example contains 2 inputs, 2 buttons Datepicker and Daterangepicker, value in Datepicker input is current day,
        ...
        datepicker.isInputValueEqual(initialState, `${newDate.toLocaleDateString('en-US', { year: 'numeric', month: '2-digit', day: '2-digit' })}`, 0);
        datepicker.isInputValueEqual(initialState, `${newDate.toLocaleDateString('en-US', { year: 'numeric', month: '2-digit', day: '2-digit' })} - ${daySevenAfterCurrent.toLocaleDateString('en-US', { year: 'numeric', month: '2-digit', day: '2-digit' })}`, 1);
    });
    ...
    it('when user chose another interval, that interval selected and shown in input', () => {
        ...
        datepicker.isDatepickerOpened(false);
        datepicker.isInputValueEqual(initialState, `${monthToChose}/${dayToChose}/${currentYearStr} - ${monthToChose}/${nextDayToChose}/${currentYearStr}`, 1);
        datepicker.clickOnBtn(initialState, 1);
...
//# sourceMappingURL=initial_state_spec.js.map