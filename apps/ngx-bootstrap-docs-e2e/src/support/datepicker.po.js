...
export class DatepickerPo extends BaseComponent {
    constructor() {
        super(...arguments);
        this.pageUrl = '/datepicker';
        this.pageTitle = 'Datepicker';
        this.ghLinkToComponent = 'https://github.com/valor-software/ngx-bootstrap/tree/development/src/datepicker';
        this.datepickerInput = 'input[bsdatepicker]';
        this.daterangepickerInput = 'input[bsdaterangepicker]';
        this.datepickerNavView = 'bs-datepicker-navigation-view';
        this.datepickerContainer = 'bs-datepicker-container';
        this.datepickerInlineContainer = 'bs-datepicker-inline-container';
        this.daterangepickerContainer = 'bs-daterangepicker-container';
        this.datepickerBodyDaysView = 'bs-days-calendar-view';
        this.datepickerBodyMonthView = 'bs-month-calendar-view';
        this.datepickerBodyYearsView = 'bs-years-calendar-view';
        this.daterangepickerQuickSelectContainer = 'bs-custom-date-view';
    ...
    clickOnDatepickerInput(baseSelector, datepickerIndex = 0) {
        cy.get(`${baseSelector} ${this.datepickerInput}`).eq(datepickerIndex).click();
    }
    clickOnDaterangepickerInput(baseSelector, dateRangeIndex = 0) {
        cy.get(`${baseSelector} ${this.daterangepickerInput}`).eq(dateRangeIndex).click();
    }
    isSelectedDateExist(picker = 'datepicker', exist, baseSelector = 'body', expectedDay) {
        const appropriateContainer = this.getAppropriateContainer(picker);
        if (!exist) {
    ...
    isVisibleMonthOrYearEqual(expectedMonth, baseSelector = 'body') {
        const appropriateContainer = this.getAppropriateContainer(baseSelector === 'body' ? 'datepicker' : 'datepickerInline');
        cy.get(`${baseSelector}>${appropriateContainer} ${this.datepickerNavView} button`).eq(1)
            .should('be.visible')
            .and('to.have.text', expectedMonth);
    }
    isVisibleDateRangePickerMonthOrYearEqual(expectedValueLeft, expectedValueRight, baseSelector = 'body') {
    ...
    clickOnQuickRangeBtn(countOfBtn) {
        cy.get(this.daterangepickerQuickSelectContainer).find('button').eq(countOfBtn).click();
    }
    isQuickSelectLastDaysApplied(datepicker, countOfBtn, baseSelector = 'body') {
    ...
    isQuickSelectRangeButtonHighlighted(rangeNumber, baseSelector = 'body') {
    ...
    isDatepickerNavigationFullyActiveAndCorrect(mode = 'date', baseSelector = 'body', expectedMonth, expectedYear) {
        const currentMonth = this.monthNames[new Date().getMonth()];
        const currentYearSrt = new Date().getFullYear().toString();
    ...
    isDateRangepickerNavigationFullyActiveAndCorrect(mode = 'date', baseSelector = 'body', expectedMonth, expectedYear) {
        const currentMonth = expectedMonth ? expectedMonth
            : this.monthNames[new Date().getMonth()];
        const nextMonth = expectedMonth ? this.monthNames[this.monthNames.indexOf(expectedMonth) + 1]
            : this.monthNames[new Date().getMonth() + 1];
        const currentYearSrt = expectedYear ? expectedYear
            : new Date().getFullYear().toString();
        const currentYearNum = expectedYear ? Number(expectedYear)
            : new Date().getFullYear();
        const nextYearStr = expectedYear ? (Number(expectedYear) + 1).toString()
    ...
    isDatePickerBodyExistAndCorrect(mode, baseSelector = 'body') {
    ...
    isDaterangePickerBodyExistAndCorrect(mode, baseSelector = 'body') {
        const bodyView = this.getBodyParams(mode).bodyView;
        const expectedLength = this.getBodyParams(mode).expectedLength;
    ...
    isDatePickerTriggerCorrect(mode, baseSelector = 'body') {
        const bodyView = this.getBodyParams(mode).bodyView;
        cy.get(`${baseSelector}>${this.datepickerContainer} ${bodyView} td`)
            .each(date => {
    ...
    isDaterangePickerTriggerCorrect(mode, baseSelector = 'body') {
        const bodyView = this.getBodyParams(mode).bodyView;
        cy.get(`${baseSelector}>${this.daterangepickerContainer} ${bodyView}`).each(bodyContainer => {
    ...
    isDatepickerOpened(opened, baseSelector = 'body') {
    ...
    isDaterangepickerOpened(opened, baseSelector = 'body') {
        cy.get(`${baseSelector}>${this.daterangepickerContainer}`).should(opened ? 'to.be.exist' : 'not.to.be.exist');
    }
    clickOnNavigation(baseSelector, navigationItem) {
        const appropriateContainer = this.getAppropriateContainer(baseSelector === 'body' ? 'datepicker' : 'datepickerInline');
    ...
    clickOnDateRangePickerNavigation(navigationItem, baseSelector = 'body') {
        switch (navigationItem) {
            case '<':
                cy.get(`${baseSelector}>${this.daterangepickerContainer} ${this.datepickerNavView}`)
                    .eq(0)
    ...
    clickOnDatepickerTableItem(mode, baseSelector = 'body', itemIndex, itemText) {
        const bodyView = this.getBodyParams(mode).bodyView;
        const appropriateContainer = this.getAppropriateContainer(baseSelector === 'body' ? 'datepicker' : 'datepickerInline');
    ...
    clickOnDatepickerWeekItem(itemIndex, itemText) {
        if (itemText === undefined) {
    ...
    clickOnDaterangePickerTableItem(mode, pickerIndex = 0, baseSelector = 'body', itemIndex, itemText) {
        const bodyView = this.getBodyParams(mode).bodyView;
        if (itemText === undefined) {
    ...
    isDatepickerStyleCorrect(expectedTheme, baseSelector = 'body') {
        let expColour;
    ...
    isMonthLocaleAppropriate(expectedLocale, pickerType = 'datepicker', baseSelector = 'body') {
        let actualMonthArr;
        ...
        cy.get(`${baseSelector}>${pickerType === 'datepicker' ? this.datepickerContainer : this.daterangepickerContainer} tbody td`)
            ...
            expect(month.text().toLowerCase()).to.contains(actualMonthArr ? actualMonthArr[monthIndex].toLowerCase() :
                new Date(2017, monthIndex)
                    .toLocaleDateString(expectedLocale, { month: 'long' })
                    .toLowerCase());
        });
    }
    isWeekdayLocaleAppropriate(expectedLocale, pickerType = 'datepicker', baseSelector = 'body') {
    ...
    isDayIntervalDisabledInCurrentMonth(minDate, maxDate, disabled) {
    ...
    isDaysDisabledInCurrentMonth(minDate, maxDate, disabled, dateRangePicker) {
    ...
    isWeekdayDisabled(disabled, weekdayIndex) {
    ...
    isDayIntervalDisabledInCurrentMonthDateRange(minDate, maxDate, disabled) {
        const minOrigin = new Date(minDate.getTime());
        const min = minDate;
        for (min; min <= maxDate && min.getMonth() === minOrigin.getMonth(); min.setDate(min.getDate() + 1)) {
    ...
    isDayIntervalDisabledInNextMonth(minDate, maxDate, disabled) {
    ...
    isDayIntervalDisabledInNextMonthDateRange(minDate, maxDate, disabled) {
        const maxOrigin = new Date(maxDate.getTime());
        const max = maxDate;
        for (max; minDate <= max && maxOrigin.getMonth() === max.getMonth(); max.setDate(max.getDate() - 1)) {
     ...
     */
    isDatepickerPlacementCorrect(baseSelector, placement, indexInput) {
        let index;
    ...
    getAppropriateContainer(picker) {
        switch (picker) {
            case 'datepicker':
...
//# sourceMappingURL=datepicker.po.js.map