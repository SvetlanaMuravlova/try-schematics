...
describe('utc offset', () => {
    ...
    it('utcOffset shorthand hours -> minutes', function () {
        var i;
        for (i = -15; i <= 15; ++i) {
            assertEq(moment().utcOffset(i).utcOffset(), i * 60, '' + i + ' -> ' + i * 60);
        }
    ...
    it('distance from the unix epoch', function () {
        ...
        zoneD.utcOffset(-480);
        assertEq(+zoneA, +zoneD, 'moment should equal moment.utcOffset(-480)');
        zoneE.utcOffset(-1000);
        assertEq(+zoneA, +zoneE, 'moment should equal moment.utcOffset(-1000)');
    });
    // TODO: use sinon
    ...
    it('cloning', function () {
        assertEq(moment().utcOffset(-120).clone().utcOffset(), -120, 'explicit cloning should retain the offset');
        assertEq(moment().utcOffset(120).clone().utcOffset(), 120, 'explicit cloning should retain the offset');
        assertEq(moment(moment().utcOffset(-120)).utcOffset(), -120, 'implicit cloning should retain the offset');
        assertEq(moment(moment().utcOffset(120)).utcOffset(), 120, 'implicit cloning should retain the offset');
    });
    it('start of / end of', function () {
        var a = moment.utc([2010, 1, 2, 0, 0, 0]).utcOffset(-450);
        assertEq(a.clone().startOf('day').hour(), 0, 'start of day should work on moments with utc offset');
        assertEq(a.clone().startOf('day').minute(), 0, 'start of day should work on moments with utc offset');
        assertEq(a.clone().startOf('hour').minute(), 0, 'start of hour should work on moments with utc offset');
        assertEq(a.clone().endOf('day').hour(), 23, 'end of day should work on moments with utc offset');
        assertEq(a.clone().endOf('day').minute(), 59, 'end of day should work on moments with utc offset');
        assertEq(a.clone().endOf('hour').minute(), 59, 'end of hour should work on moments with utc offset');
    });
...
//# sourceMappingURL=utc_offset.spec.js.map