...
describe('create', () => {
    it('array', function () {
        ...
        assertEq(+moment(new Date(2010, 1, 14, 15, 25, 50, 125)), +moment([2010, 1, 14, 15, 25, 50, 125]), 'constructing with array === constructing with new Date()');
    });
    ...
    */
    ...
    it('iso format 24hrs', function () {
        assertEq(moment('2014-01-01T24:00:00.000').format('YYYY-MM-DD[T]HH:mm:ss.SSS'), '2014-01-02T00:00:00.000', 'iso format with 24:00 localtime');
        assertEq(moment.utc('2014-01-01T24:00:00.000').format('YYYY-MM-DD[T]HH:mm:ss.SSS'), '2014-01-02T00:00:00.000', 'iso format with 24:00 utc');
    });
    ...
    it('defaulting to current date', function () {
        const now = moment();
        assertEq(moment('12:13:14', 'hh:mm:ss').format('YYYY-MM-DD hh:mm:ss'), now.clone().hours(12).minutes(13).seconds(14).format('YYYY-MM-DD hh:mm:ss'), 'given only time default to current date');
        assertEq(moment('05', 'DD').format('YYYY-MM-DD'), now.clone().date(5).format('YYYY-MM-DD'), 'given day of month default to current month, year');
        assertEq(moment('05', 'MM').format('YYYY-MM-DD'), now.clone().month(4).date(1).format('YYYY-MM-DD'), 'given month default to current year');
        assertEq(moment('1996', 'YYYY').format('YYYY-MM-DD'), now.clone().year(1996).month(0).date(1).format('YYYY-MM-DD'), 'given year do not default');
    });
    ...
    it('parsing iso', function () {
        let offset = moment([2011, 9, 8]).utcOffset(), pad = function (input) {
            if (input < 10) {
                return '0' + input;
            }
            return '' + input;
        ...
        for (i = 0; i < formats.length; i++) {
            assertEq(moment(formats[i][0]).format('YYYY-MM-DDTHH:mm:ss.SSSZ'), formats[i][1], 'moment should be able to parse ISO ' + formats[i][0]);
            assertEq(moment(formats[i][0], moment.ISO_8601).format('YYYY-MM-DDTHH:mm:ss.SSSZ'), formats[i][1], 'moment should be able to parse specified ISO ' + formats[i][0]);
            assertEq(moment(formats[i][0], moment.ISO_8601, true).format('YYYY-MM-DDTHH:mm:ss.SSSZ'), formats[i][1], 'moment should be able to parse specified strict ISO ' + formats[i][0]);
        }
    });
    ...
    it('parsing invalid string weekdays', function () {
        assertEq(false, moment('a', 'dd').isValid(), 'dd with invalid weekday, non-strict');
        assertEq(false, moment('a', 'dd', true).isValid(), 'dd with invalid weekday, strict');
        assertEq(false, moment('a', 'ddd').isValid(), 'ddd with invalid weekday, non-strict');
        assertEq(false, moment('a', 'ddd', true).isValid(), 'ddd with invalid weekday, strict');
        assertEq(false, moment('a', 'dddd').isValid(), 'dddd with invalid weekday, non-strict');
        assertEq(false, moment('a', 'dddd', true).isValid(), 'dddd with invalid weekday, strict');
    });
