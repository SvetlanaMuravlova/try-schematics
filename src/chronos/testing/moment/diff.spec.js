import { assertEq, assertOk } from '../test-helpers';
...
describe('diff', () => {
    ...
    it('diff across DST', function () {
        ...
        assertEq(b.diff(a, 'milliseconds', true), 12 * 60 * 60 * 1000, 'ms diff across DST');
        assertEq(b.diff(a, 'seconds', true), 12 * 60 * 60, 'second diff across DST');
        assertEq(b.diff(a, 'minutes', true), 12 * 60, 'minute diff across DST');
        assertEq(b.diff(a, 'hours', true), 12, 'hour diff across DST');
        equal(b.diff(a, 'days', true), (12 - dst.diff) / 24, 'day diff across DST');
        equal(b.diff(a, 'weeks', true), (12 - dst.diff) / 24 / 7, 'week diff across DST');
        assertOk(0.95 / (2 * 31) < b.diff(a, 'months', true), 'month diff across DST, lower bound');
        assertOk(b.diff(a, 'month', true) < 1.05 / (2 * 28), 'month diff across DST, upper bound');
        assertOk(0.95 / (2 * 31 * 12) < b.diff(a, 'years', true), 'year diff across DST, lower bound');
        assertOk(b.diff(a, 'year', true) < 1.05 / (2 * 28 * 12), 'year diff across DST, upper bound');
        a = dst.moment;
        b = a.clone().utc().add(12 + dst.diff, 'hours').local();
        daysInMonth = (a.daysInMonth() + b.daysInMonth()) / 2;
        assertEq(b.diff(a, 'milliseconds', true), (12 + dst.diff) * 60 * 60 * 1000, 'ms diff across DST');
        assertEq(b.diff(a, 'seconds', true), (12 + dst.diff) * 60 * 60, 'second diff across DST');
        assertEq(b.diff(a, 'minutes', true), (12 + dst.diff) * 60, 'minute diff across DST');
        assertEq(b.diff(a, 'hours', true), (12 + dst.diff), 'hour diff across DST');
        assertEq(b.diff(a, 'days', true), 12 / 24, 'day diff across DST');
        equal(b.diff(a, 'weeks', true), 12 / 24 / 7, 'week diff across DST');
        assertOk(0.95 / (2 * 31) < b.diff(a, 'months', true), 'month diff across DST, lower bound');
        assertOk(b.diff(a, 'month', true) < 1.05 / (2 * 28), 'month diff across DST, upper bound');
        assertOk(0.95 / (2 * 31 * 12) < b.diff(a, 'years', true), 'year diff across DST, lower bound');
        assertOk(b.diff(a, 'year', true) < 1.05 / (2 * 28 * 12), 'year diff across DST, upper bound');
    ...
    it('exact month diffs', function () {
        ...
        for (m1 = 0; m1 < 12; ++m1) {
            for (m2 = m1; m2 < 12; ++m2) {
                assertEq(moment([2013, m2, 15]).diff(moment([2013, m1, 15]), 'months', true), m2 - m1, 'month diff from 2013-' + m1 + '-15 to 2013-' + m2 + '-15');
            }
        }
    });
