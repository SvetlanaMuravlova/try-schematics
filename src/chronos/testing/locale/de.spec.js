...
describe('locale: de', () => {
    ...
    it('parse', function () {
        var _tests = 'Januar Jan._Februar Feb._März März_April Apr._Mai Mai_Juni Juni_Juli Juli_August Aug._September Sep._Oktober Okt._November Nov._Dezember Dez.'.split('_'), i;
    ...
    it('from', function () {
        ...
        assertEq(start.from(moment([2007, 1, 28]).add({ s: 44 }), true), 'ein paar Sekunden', '44 seconds = a few seconds');
        assertEq(start.from(moment([2007, 1, 28]).add({ s: 45 }), true), 'eine Minute', '45 seconds = a minute');
        assertEq(start.from(moment([2007, 1, 28]).add({ s: 89 }), true), 'eine Minute', '89 seconds = a minute');
        ...
        assertEq(start.from(moment([2007, 1, 28]).add({ m: 45 }), true), 'eine Stunde', '45 minutes = an hour');
        assertEq(start.from(moment([2007, 1, 28]).add({ m: 89 }), true), 'eine Stunde', '89 minutes = an hour');
        assertEq(start.from(moment([2007, 1, 28]).add({ m: 90 }), true), '2 Stunden', '90 minutes = 2 hours');
        assertEq(start.from(moment([2007, 1, 28]).add({ h: 5 }), true), '5 Stunden', '5 hours = 5 hours');
        assertEq(start.from(moment([2007, 1, 28]).add({ h: 21 }), true), '21 Stunden', '21 hours = 21 hours');
        assertEq(start.from(moment([2007, 1, 28]).add({ h: 22 }), true), 'ein Tag', '22 hours = a day');
        assertEq(start.from(moment([2007, 1, 28]).add({ h: 35 }), true), 'ein Tag', '35 hours = a day');
        assertEq(start.from(moment([2007, 1, 28]).add({ h: 36 }), true), '2 Tage', '36 hours = 2 days');
        assertEq(start.from(moment([2007, 1, 28]).add({ d: 1 }), true), 'ein Tag', '1 day = a day');
        assertEq(start.from(moment([2007, 1, 28]).add({ d: 5 }), true), '5 Tage', '5 days = 5 days');
        assertEq(start.from(moment([2007, 1, 28]).add({ d: 25 }), true), '25 Tage', '25 days = 25 days');
        assertEq(start.from(moment([2007, 1, 28]).add({ d: 26 }), true), 'ein Monat', '26 days = a month');
        assertEq(start.from(moment([2007, 1, 28]).add({ d: 30 }), true), 'ein Monat', '30 days = a month');
        assertEq(start.from(moment([2007, 1, 28]).add({ d: 43 }), true), 'ein Monat', '43 days = a month');
        assertEq(start.from(moment([2007, 1, 28]).add({ d: 46 }), true), '2 Monate', '46 days = 2 months');
        assertEq(start.from(moment([2007, 1, 28]).add({ d: 74 }), true), '2 Monate', '75 days = 2 months');
        assertEq(start.from(moment([2007, 1, 28]).add({ d: 76 }), true), '3 Monate', '76 days = 3 months');
        assertEq(start.from(moment([2007, 1, 28]).add({ M: 1 }), true), 'ein Monat', '1 month = a month');
        assertEq(start.from(moment([2007, 1, 28]).add({ M: 5 }), true), '5 Monate', '5 months = 5 months');
        assertEq(start.from(moment([2007, 1, 28]).add({ d: 345 }), true), 'ein Jahr', '345 days = a year');
        assertEq(start.from(moment([2007, 1, 28]).add({ d: 548 }), true), '2 Jahre', '548 days = 2 years');
        assertEq(start.from(moment([2007, 1, 28]).add({ y: 1 }), true), 'ein Jahr', '1 year = a year');
        assertEq(start.from(moment([2007, 1, 28]).add({ y: 5 }), true), '5 Jahre', '5 years = 5 years');
    });
    it('suffix', function () {
        assertEq(moment(30000).from(0), 'in ein paar Sekunden', 'prefix');
        assertEq(moment(0).from(30000), 'vor ein paar Sekunden', 'suffix');
    });
    it('fromNow', function () {
        assertEq(moment().add({ s: 30 }).fromNow(), 'in ein paar Sekunden', 'in a few seconds');
        assertEq(moment().add({ d: 5 }).fromNow(), 'in 5 Tagen', 'in 5 days');
    });
    it('calendar day', function () {
        var a = moment().hours(12).minutes(0).seconds(0);
        assertEq(moment(a).calendar(), 'heute um 12:00 Uhr', 'today at the same time');
        assertEq(moment(a).add({ m: 25 }).calendar(), 'heute um 12:25 Uhr', 'Now plus 25 min');
        assertEq(moment(a).add({ h: 1 }).calendar(), 'heute um 13:00 Uhr', 'Now plus 1 hour');
        assertEq(moment(a).add({ d: 1 }).calendar(), 'morgen um 12:00 Uhr', 'tomorrow at the same time');
        assertEq(moment(a).subtract({ h: 1 }).calendar(), 'heute um 11:00 Uhr', 'Now minus 1 hour');
        assertEq(moment(a).subtract({ d: 1 }).calendar(), 'gestern um 12:00 Uhr', 'yesterday at the same time');
    });
    it('calendar next week', function () {
        var i, m;
        for (i = 2; i < 7; i++) {
            m = moment().add({ d: i });
            assertEq(m.calendar(), m.format('dddd [um] LT [Uhr]'), 'Today + ' + i + ' days current time');
            m.hours(0).minutes(0).seconds(0).milliseconds(0);
            assertEq(m.calendar(), m.format('dddd [um] LT [Uhr]'), 'Today + ' + i + ' days beginning of day');
            m.hours(23).minutes(59).seconds(59).milliseconds(999);
            assertEq(m.calendar(), m.format('dddd [um] LT [Uhr]'), 'Today + ' + i + ' days end of day');
        }
    });
    it('calendar last week', function () {
        ...
        for (i = 2; i < 7; i++) {
            ...
            assertEq(m.calendar(), m.format('[letzten] dddd [um] LT [Uhr]'), 'Today + ' + i + ' days current time');
            m.hours(0).minutes(0).seconds(0).milliseconds(0);
            assertEq(m.calendar(), m.format('[letzten] dddd [um] LT [Uhr]'), 'Today + ' + i + ' days beginning of day');
            m.hours(23).minutes(59).seconds(59).milliseconds(999);
            assertEq(m.calendar(), m.format('[letzten] dddd [um] LT [Uhr]'), 'Today + ' + i + ' days end of day');
        }
    });
