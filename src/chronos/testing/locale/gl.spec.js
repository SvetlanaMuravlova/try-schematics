...
import { glLocale } from '../../i18n/gl';
// localeModule('en');
describe('locale: gl', () => {
    beforeAll(() => {
        moment.locale('gl', glLocale);
    });
    afterAll(() => {
        moment.locale('en');
    });
    // localeModule('es');
    it('parse', function () {
        var _tests = 'xaneiro xan._febreiro feb._marzo mar._abril abr._maio mai._xuño xuñ._xullo xul._agosto ago._setembro set._outubro out._novembro nov._decembro dec.'.split('_'), i;
    ...
    it('format week', function () {
        var expected = 'domingo dom. do_luns lun. lu_martes mar. ma_mércores mér. mé_xoves xov. xo_venres ven. ve_sábado sáb. sá'.split('_'), i;
        for (i = 0; i < expected.length; i++) {
    ...
    it('from', function () {
        ...
        assertEq(start.from(moment([2007, 1, 28]).add({ s: 44 }), true), 'uns segundos', '44 seconds = a few seconds');
        ...
        assertEq(start.from(moment([2007, 1, 28]).add({ m: 45 }), true), 'unha hora', '45 minutes = an hour');
        assertEq(start.from(moment([2007, 1, 28]).add({ m: 89 }), true), 'unha hora', '89 minutes = an hour');
        ...
        assertEq(start.from(moment([2007, 1, 28]).add({ d: 345 }), true), 'un ano', '345 days = a year');
        assertEq(start.from(moment([2007, 1, 28]).add({ d: 548 }), true), '2 anos', '548 days = 2 years');
        assertEq(start.from(moment([2007, 1, 28]).add({ y: 1 }), true), 'un ano', '1 year = a year');
        assertEq(start.from(moment([2007, 1, 28]).add({ y: 5 }), true), '5 anos', '5 years = 5 years');
    });
    it('suffix', function () {
        assertEq(moment(30000).from(0), 'en uns segundos', 'prefix');
        assertEq(moment(0).from(30000), 'fai uns segundos', 'suffix');
    });
    it('now from now', function () {
        assertEq(moment().fromNow(), 'fai uns segundos', 'now from now should display as in the past');
    });
    it('fromNow', function () {
        assertEq(moment().add({ s: 30 }).fromNow(), 'en uns segundos', 'en uns segundos');
        assertEq(moment().add({ d: 5 }).fromNow(), 'en 5 días', 'en 5 días');
    });
    it('calendar day', function () {
        var a = moment().hours(12).minutes(0).seconds(0);
        assertEq(moment(a).calendar(), 'hoxe ás 12:00', 'today at the same time');
        assertEq(moment(a).add({ m: 25 }).calendar(), 'hoxe ás 12:25', 'Now plus 25 min');
        assertEq(moment(a).add({ h: 1 }).calendar(), 'hoxe ás 13:00', 'Now plus 1 hour');
        assertEq(moment(a).add({ d: 1 }).calendar(), 'mañan ás 12:00', 'tomorrow at the same time');
        assertEq(moment(a).add({ d: 1, h: -1 }).calendar(), 'mañan ás 11:00', 'tomorrow minus 1 hour');
        assertEq(moment(a).subtract({ h: 1 }).calendar(), 'hoxe ás 11:00', 'Now minus 1 hour');
        assertEq(moment(a).subtract({ d: 1 }).calendar(), 'onte ás 12:00', 'yesterday at the same time');
    });
    it('calendar next week', function () {
        var i, m;
        for (i = 2; i < 7; i++) {
            m = moment().add({ d: i });
            assertEq(m.calendar(), m.format('dddd [á' + ((m.hours() !== 1) ? 's' : '') + '] LT'), 'Today + ' + i + ' days current time');
            m.hours(0).minutes(0).seconds(0).milliseconds(0);
            assertEq(m.calendar(), m.format('dddd [á' + ((m.hours() !== 1) ? 's' : '') + '] LT'), 'Today + ' + i + ' days beginning of day');
            m.hours(23).minutes(59).seconds(59).milliseconds(999);
            assertEq(m.calendar(), m.format('dddd [á' + ((m.hours() !== 1) ? 's' : '') + '] LT'), 'Today + ' + i + ' days end of day');
        }
    });
    it('calendar last week', function () {
        ...
        for (i = 2; i < 7; i++) {
            ...
            assertEq(m.calendar(), m.format('[o] dddd [pasado á' + ((m.hours() !== 1) ? 's' : '') + '] LT'), 'Today - ' + i + ' days current time');
            m.hours(0).minutes(0).seconds(0).milliseconds(0);
            assertEq(m.calendar(), m.format('[o] dddd [pasado á' + ((m.hours() !== 1) ? 's' : '') + '] LT'), 'Today - ' + i + ' days beginning of day');
            m.hours(23).minutes(59).seconds(59).milliseconds(999);
            assertEq(m.calendar(), m.format('[o] dddd [pasado á' + ((m.hours() !== 1) ? 's' : '') + '] LT'), 'Today - ' + i + ' days end of day');
        }
    });
...
//# sourceMappingURL=gl.spec.js.map