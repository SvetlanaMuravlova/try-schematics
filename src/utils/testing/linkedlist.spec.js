...
describe('Linked List. Overridden (from Array) methods.', () => {
    ...
    it('Positive check some()', () => {
        expect(list.some((item) => item === '2')).toBeTruthy();
    });
    it('Negative check some()', () => {
        expect(list.some((item) => item === '6')).toBeFalsy();
    });
    it('Negative check every()', () => {
        expect(list.every((item) => item === '2')).toBeFalsy();
    });
    it('Positive check every()', () => {
        expect(list.every((item) => item !== '0')).toBeTruthy();
    });
});
